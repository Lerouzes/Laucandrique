...
export function Header({ user, profile }: { user: User, profile: Profile | null }) {
    ...
    return (
        <header className="flex h-16 items-center justify-end border-b border-white/20 bg-[#0f3f74]/92 px-6">
            <div className="flex items-center gap-4">
                ...
                <Avatar className="h-9 w-9 border border-white/25">
                    <AvatarFallback className="bg-white/20 text-white font-medium">{initials}</AvatarFallback>
                </Avatar>
                <Button variant="ghost" size="icon" onClick={handleSignOut} className="text-white/75 hover:text-white hover:bg-white/15">
