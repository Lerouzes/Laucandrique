...
function QuoteRow({ quote }: { quote: any }) {
    ...
    const scheduledDateLabel = formatSafeDate(scheduledDate)
    const createdAtLabel = formatSafeDate(quote.created_at)
        ...
        >
            ...
            <TableCell className="text-right">
                <div className="flex items-center justify-end gap-1" onClick={e => e.stopPropagation()}>
                        ...
                        <Popover>
                            <PopoverTrigger render={<Button size="sm" variant="ghost" className="h-7 px-2 text-cyan-300 hover:text-cyan-200 hover:bg-cyan-950/50" title={scheduledDate ? 'Modifier la date de début' : 'Planifier ce projet'} />} >
                                ...
                                <span className="ml-1 hidden md:inline">
                                    {scheduledDateLabel || 'Planifier'}
                                </span>
                            </PopoverTrigger>
                            <PopoverContent align="end" sideOffset={6} className="w-auto p-2 bg-zinc-950 border-zinc-800">
                                <Calendar
                                    mode="single"
                                    selected={scheduledDateLabel && scheduledDate ? new Date(scheduledDate) : undefined}
                                    onSelect={(date) => {
                                        if (!date) return
                                        startTransition(async () => {
                                            try {
                    ...
                    ) : quote.status === 'completed' && scheduledDateLabel ? (
                        <span className="inline-flex items-center gap-1 text-blue-300" title="Projet complété">
                            <CalendarDays className="h-4 w-4" />
                            <span className="hidden md:inline">{scheduledDateLabel}</span>
                        </span>
                    ) : (
