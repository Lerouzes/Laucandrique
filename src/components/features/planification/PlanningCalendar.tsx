    ...
    return (
        <div className="flex flex-col h-full space-y-4">
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center justify-between gap-2 pt-1 border-t border-zinc-800/40 flex-wrap">
                                                ...
                                                <div className="flex items-center gap-0.5">
                                                    ...
                                                    {(project.status === 'completed' || project.status === 'deferred') && (
                                                        <button
                                                            onClick={(e) => { e.stopPropagation(); handleStatusChange(project.id, 'unplanned') }}
                                                    ...
                                                    {/* Restore / Uncancel button specifically for cancelled status */}
                                                    {project.status === 'cancelled' && (
                                                        <button
                                                            onClick={(e) => { 
                                                                e.stopPropagation(); 
                                                                handleStatusChange(project.id, project.start_date ? 'planned' : 'unplanned') 
                                                            }}
                                                            className="text-zinc-500 hover:text-emerald-450 transition-colors p-1"
                                                            title="Rétablir le projet (Désannuler)"
                                                        >
                                                            <RefreshCw className="h-3.5 w-3.5" />
                                                        </button>
                                                    )}
            ...
            <Dialog open={isDetailModalOpen} onOpenChange={setIsDetailModalOpen}>
                <DialogContent className="sm:max-w-6xl w-[90vw] max-w-[90vw] bg-zinc-950 border border-zinc-800 text-zinc-100 max-h-[90vh] overflow-y-auto p-0 rounded-2xl shadow-2xl">
                    {loadingDetails ? (
