...
export async function getQuotes(query?: string, statusFilter?: string) {
    ...
    let request = supabase
        .from('quotes')
        .select('*, clients(id, full_name, company_name, manager_id, status, departure_date, contracts(active)), managers(first_name,last_name,team_id), contractors(id, full_name, color), projects(start_date, completed_at, status)')
    ...
    try {
        ...
        for (const row of rows) {
            ...
            if (row.import_action === 'update' && quoteId) {
                ...
                // Check if quote_number already exists, to avoid unique constraint violations
            ...
            // 3. Resolve or Create Project for Approved/Completed Quotes
            const isApprovedOrCompleted = row.status === 'approved' || row.status === 'completed'
        ...
        console.error("Exception in confirmBulkQuoteImportAction:", err)
        return {
            success: false,
