...
export async function createQuoteAction(quoteData: any, itemsData: any[], imagesData: any[]) {
    ...
    if (quoteError && (quoteError.message.includes('manager_id') || quoteError.message.includes('project_type') || quoteError.message.includes('contractor_id'))) {
...
export async function updateQuoteStatus(quoteId: string, status: 'approved' | 'denied', clientId: string, titleStr: string, durDays: number) {
    ...
    if (status === 'approved') {
        ...
        if (inserted.error && (inserted.error.message.includes('project_type') || inserted.error.message.includes('contractor_id'))) {
