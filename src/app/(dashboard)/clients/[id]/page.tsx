  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <form action={async (fd) => { 'use server'; await updateClientAction(id, fd) }} className="grid grid-cols-2 gap-3 rounded-lg border border-zinc-800 bg-zinc-900 p-4">
        ...
        <select name="manager_id" defaultValue={client.manager_id || ''} className="h-10 rounded-md border border-zinc-800 bg-zinc-950 px-3 text-zinc-100">
      ...
      <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-4">
        ...
        <div className="space-y-2 text-sm">
          {clientQuotes.length === 0 ? <p className="text-zinc-400">Aucune soumission.</p> : clientQuotes.map((q: any) => (
