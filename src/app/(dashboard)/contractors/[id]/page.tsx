...
export default async function ContractorDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className='space-y-6 max-w-3xl'>
    <div>
      <Link href='/contractors' className='text-sm text-zinc-500 hover:text-zinc-900'>← Retour aux contracteurs</Link>
      <h2 className='text-2xl font-bold text-zinc-900 mt-2'>{contractor.full_name}</h2>
    </div>
    <form action={async (fd) => { 'use server'; await updateContractorAction(id, fd) }} className='grid grid-cols-2 gap-3 rounded-lg border bg-white p-4'>
      ...
      <div className='col-span-2 grid grid-cols-2 gap-2'>
        {SKILLS.map((s) => <label key={s} className='text-sm flex items-center gap-2'><input type='checkbox' name='skills' value={s} defaultChecked={(contractor.skills || []).includes(s)} />{s}</label>)}
      </div>
    ...
    <div className='rounded-lg border bg-white p-4'>
      <h3 className='font-semibold mb-3'>Historique des soumissions</h3>
      <div className='space-y-2 text-sm'>{related.length === 0 ? 'Aucune soumission.' : related.map((q: any) => <Link key={q.id} href={`/quotes/${q.id}`} className='block hover:underline'>#{q.quote_number} — {q.title} ({q.status})</Link>)}</div>
    </div>
  </div>
}
